...
Look at dates built vs date of event - how many buildings re-built following fire...
```{r}
library(tidyverse)
library(lubridate)

# FENZ dataset = 60409 variables
#1. pull out year from incident date
buildings_fenz_res_fatalities$incident_year <- as.POSIXct(buildings_fenz_res_fatalities$Date_Time, format = "%d/%m/%Y %H:%M:%S")
buildings_fenz_res_fatalities$incident_year <- as.integer(format(buildings_fenz_res_fatalities$incident_year, format="%Y"))
#2. calculate difference between event and build year - 0 = same year, +ve = newer, -ve = older
buildings_fenz_res_fatalities$year_diff <- buildings_fenz_res_fatalities$Age - buildings_fenz_res_fatalities$incident_year 
#3. plot
hist(buildings_fenz_res_fatalities$year_diff)
#4.
sum(buildings_fenz_res_fatalities$year_diff >0 , na.rm=TRUE) 
sum(buildings_fenz_res_fatalities$year_diff >=0 , na.rm=TRUE)
sum(buildings_fenz_res_fatalities$year_diff >0 , na.rm=TRUE) / 213
sum(buildings_fenz_res_fatalities$year_diff >=0 , na.rm=TRUE) / 213
